...
void HitmanProc::BeginOfRun(RAT::DS::Run* run) {
  ...
  // Set up relative reference PtpSimulations/models
  std::string path = "";
  for (auto dir : Ptp::model_directories) {
  ...
  // Check to see if model exists as an 'approved' PtpSimulations model, otherwise use absolute path
  if (!std::filesystem::exists(path)) {
    RAT::warn << "Could not find HITMAN model at PtpSimulations/models" + directory << newline;
